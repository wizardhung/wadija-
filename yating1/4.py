...
PIPELINE = "asr-zh-en-nan"
TOKEN_URL = "https://asr.api.yating.tw/api/v1/token"
...
def detect_language(text: str) -> str:
    # 台語關鍵詞：請依實際需要擴充
    taiwanese_keywords = [
        "欲", "敢會", "有影無", "煞", "無啥物", "食飯", "咱", "啊你", "汝", "攏", "恁",
        "嘛會", "歹勢", "共款", "輸贏", "講", "麥", "哩", "毋是", "食飽", "食未", "去佗位", "佮意"
    ]
    ...
    # 如果出現台語關鍵詞，就當作是台語
    if any(kw in text for kw in taiwanese_keywords):
        return "台語"
        ...
        try:
            ...
            if frame and frame.opcode == websocket.ABNF.OPCODE_TEXT:
                ...
                if pipe.get("asr_final"):
                    ...
                    print(json.dumps(payload, indent=2, ensure_ascii=False))  # 加這行來看整包資料

                    segments = pipe.get("segments", [])
